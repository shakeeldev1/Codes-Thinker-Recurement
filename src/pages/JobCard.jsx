...
const JobCard = ({ job, onClick }) => {
    return (
        <motion.div
            ...
            className="bg-white rounded-2xl hover:shadow-xl shadow-amber-200 shadow-lg transition-all duration-300 
            border border-[#060145]/50 cursor-pointer flex flex-col overflow-hidden 
            w-full h-full max-w-sm mx-auto"
        >
            {/* Header */}
            <div className="bg-gradient-to-r from-[#060145] to-[#F59C20] text-white px-5 py-6">
                <h3 className="text-lg sm:text-xl font-bold truncate">{job.title}</h3>
                <p className="text-sm opacity-90 truncate">{job.company}</p>
            </div>
                ...
                <div className="flex flex-wrap gap-2">
                    {job.skills.slice(0, 3).map((skill, i) => (
                        <motion.span
                            ...
                            className="px-2 py-1 bg-[#060145]/10 text-[#060145] text-xs font-medium rounded-md"
                        >
                            {skill}
                        </motion.span>
                    ))}
            ...
            {/* Footer */}
            <div className="px-5 py-3 bg-[#060145] text-white text-xs sm:text-sm flex items-center justify-between">
                ...
                {job.remote && (
                    <div className="flex items-center gap-1 px-2 py-1 bg-[#F59C20] text-[#060145] text-[10px] sm:text-xs font-semibold rounded-md">
